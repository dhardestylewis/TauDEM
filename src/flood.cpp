        ...
        bool is_4Point,bool use_mask,char *maskfile)  // these three added by arb, 5/31/11
{
    ...
    MPI_Init(NULL,NULL);{
        ...
        linearpart<short>* maskPartition;

        if (use_mask) {
            maskPartition = new linearpart<short>(totalX, totalY, dxA, dyA, MPI_SHORT, *(short *) depmask->getNodata());
        }
        ...
        if(verbose)
        {
            ...
            long nxm=nx/2;
            long nym=ny/2;
            float val = elevDEM.getData(nxm,nym);
        ...
        while(planchon.isInPartition(i,j)) { 
            ...
            if(!planchon.isNodata(i,j) && planchon.getData(i,j) > elevDEM.getData(i,j)){
                ...
                {			
                    ...
                    if(planchon.getData(i,j) > neighborFloat /* && elevDEM.getData(i,j) < neighborFloat */){
        ...
        while(!finished){
            ...
            printf("rank %d: share planchon @%d\n", rank, millis() - start);
            planchon.share();
            s1.clear();
            ...
            printf("rank %d: iter s2 %d @%d\n", rank, s2.size(), millis() - start);
            ...
            printf("rank %d: ringterm start @%d\n", rank, millis() - start);
            finished = planchon.ringTerm(finished);
            printf("rank %d: ringterm done @%d\n", rank, millis() - start);
            ...
            if(verbose)
            {
                ...
                long nxm=nx/2;
                long nym=ny/2;
                float val = planchon.getData(nxm,nym);
